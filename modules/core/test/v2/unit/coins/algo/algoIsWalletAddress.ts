...
describe('Algo class', function () {
  ...
  describe('for method isWalletAddress', () => {
    ...
    const makeVerifyAddressOptions = (address: string, rootAddress: string, bitgoPubKey?: string): VerifyAlgoAddressOptions => ({
      address,
      chain: 0,
      index: 0,
      coin: 'talgo',
    ...
    [
      ...
      {
        title: 'should not validate an invalid formatted address',
        address: 'GD64YIY3TWGDMCNPP553DZPPR6LDUSFQOIJVFDPPXWEG3FVOJ',
        throws: /invalid address/,
      },
      {
        title: 'should not validate an invalid checksum address',
        address: 'GD64YIY3TWGDMCNPP553DZPPR6LDUSFQOIJVFDPPXWEG3F999',
        throws: /invalid address/,
      },
    ].forEach(({ title, address, expected, bitgoPubKey, throws }) => {
      it(title, () => {
        // GIVEN parameter options for created address
        const params = makeVerifyAddressOptions(address, ROOT_ADDRESS, bitgoPubKey);
